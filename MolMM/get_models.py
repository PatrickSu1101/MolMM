...
def get_models(args):
    ...
    data_module=DataModule(name_list,num_workers=3,batch_size=batch_size,task_list=task_list)
    data_module.prepare_data()
    data_module.setup("fit")
    
    trainer_test=pl.Trainer(
        devices=devices,
        logger=CSVLogger(result_dir,name=name)
    )
    ...
    for i, (train_data,val_data,test_data,ft_data,ft_val_data,ft_test_data) in enumerate(zip(
        ...
        if 'stop' not in name and 'pure' not in name:
            print('Begin to fine-tune, Loading PTMs,',path)
            best_model=MultiTaskModel.load_from_checkpoint(path)
        else:
            print('Begin to build PTMs')
            if 'pure' not in name:
                early_stopping = \
                    EarlyStopping(monitor=monitor,patience=patience,mode=mode,min_delta=1e-3)
                checkpoint_callback = ModelCheckpoint(monitor=monitor,mode=mode,save_top_k=1)
                trainer = pl.Trainer(
                    #strategy='ddp_find_unused_parameters_true',
                    devices=devices,
